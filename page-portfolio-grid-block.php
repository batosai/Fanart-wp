<?php 
/*
Template Name: Portfolio - Grid Style 1 
*/
...
?>
    <div class="fullscreen_block grid_style">
		<?php 
			...
			if (!isset($gt3_theme_pagebuilder['fs_portfolio']['filter']) || $gt3_theme_pagebuilder['fs_portfolio']['filter'] == 'on') {
				$compile = '';
				$compile .= '<div class="short_filter">'.showPortCats($post_type_terms_filter).'</div>';
				echo $compile;		
			}
		?>
					...
					<div class="fw_preview_wrapper">
                    	<div class="gallery_item_wrapper">
                            <a href="<?php echo $linkToTheWork; ?>" <?php echo $target; ?>>
                                <img src="<?php echo aq_resize($featured_image[0], "540", "350", true, true, true); ?>" alt="" class="fw_featured_image" width="540" height="350">
	...
	<?php if (isset($gt3_theme_pagebuilder['portfolio']['port_type']) && $gt3_theme_pagebuilder['portfolio']['port_type'] == 'port_isotope') {?>
        function get_works() {
            gt3_get_isotope_posts("port", <?php gt3_the_theme_option('fw_port_per_page'); ?>, posts_already_showed, "port_grid_template_isotope", ".fs_blog_module" <?php echo (isset($selected_categories) && strlen($selected_categories)>0 ? ', "' . $selected_categories.'"' : "") ?>);
	...
	<?php } else {?>
        function get_works() {
            ...
            gt3_get_blog_posts("port", <?php gt3_the_theme_option('fw_port_per_page'); ?>, posts_already_showed, "port_grid_template", ".fs_blog_module" <?php echo (isset($selected_categories) && strlen($selected_categories)>0 ? ', "' . $selected_categories.'"' : "") ?>);
            posts_already_showed = posts_already_showed + <?php gt3_the_theme_option('fw_port_per_page'); ?>;
        }	
        jQuery(document).ready(function () {
