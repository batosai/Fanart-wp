...
jQuery(document).ready(function () {
    ...
    jQuery(document).on('click', '.slider_type .available_media_item', function () {
        var available_media_item_this_attach_id = jQuery(this).find('.previmg').attr('data-attach-id');
        var available_media_item_this_thumburl = jQuery(this).find('.previmg').attr('data-thumb-url');
    ...
    jQuery(document).on('click', '.pb-module', function () {
        ...
        jQuery.post(ajaxurl, data, function (response) {
			...
			tinymce.execCommand('mceAddEditor', false, tinyrand);
        });
    });
    /* END ADD MODULE */
    ...
    jQuery(document).on('click', '.all_part', function () {
        ...
        if (jQuery(this).attr("checked")) {jQuery(this).parents(".checkbox_wrapper").find(".category_part").removeAttr("checked");}
    });
    ...
    jQuery(document).on("click", ".social_list_for_select2 i", function () {
        var clicked_icon = jQuery(this).attr("data-icon-code");
        var rnd_team = getRandomInt(1000, 9999);
        jQuery(this).parents(".hright").find(".added_icons").append("<div class='stand_iconsweet new_added ui-state-default' style='display:none'><span class='stand_icon-container'><i class='stand_icon "+clicked_icon+"'></i></span><input type='hidden' name='pagebuilder[page_settings][icons]["+clicked_icon+rnd_team+"][data-icon-code]' value='"+clicked_icon+"'><input type='text' class='icon_name' name='pagebuilder[page_settings][icons]["+clicked_icon+rnd_team+"][name]' value='' placeholder='Give some name'><input type='text' class='icon_link' name='pagebuilder[page_settings][icons]["+clicked_icon+rnd_team+"][link]' value='' placeholder='Give some link'><span class='remove_me'><i class='stand_icon icon-times'></i></span></div>");
		jQuery('.new_added').slideDown(300).removeClass('new_added');
        reactivate_color_picker();
        reactivate_sortable();
    });
