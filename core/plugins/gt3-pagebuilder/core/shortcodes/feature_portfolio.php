        ...
        ';
            ...
            while ($wp_query->have_posts()) : $wp_query->the_post();
                ...
                if (strlen($featured_image[0]) > 0) {
                    $featured_image_url = aq_resize($featured_image[0], "540", "368", true, true, true);
                    $full_image_url = $featured_image[0];
                    $featured_image_full = '<div class="img_block wrapped_img"><a class="featured_ico_link" href="' . get_permalink(get_the_ID()) . '"><img alt="" width="540" height="392" src="' . $featured_image_url . '" /><div class="featured_item_fadder"></div><span class="gallery_ico"><i class="stand_icon icon-eye"></i></span></a></div>';
                } else {
                    $featured_image_full = '';
                }
				...
				$echoallterm = '';
                $echoallterm_array = array();
                if (is_array($new_term_list)) {
                    foreach ($new_term_list as $term) {
                        $echoallterm_array[] = $term->name;
                    }
                } else {
                    $tempname = 'Uncategorized';
                }

                if (isset($echoallterm_array) && is_array($echoallterm_array)) {$tempname = implode($echoallterm_array, ", ");}
							...
							<div class="item_wrapper">
								...
								<div class="featured_items_body">
									...
									<div class="featured_item_content">
									   ' . smarty_modifier_truncate($post_excerpt, 80) . '
										<a class="morelink" href="' . get_permalink(get_the_ID()) . '">' . __('Read more', 'theme_localization') . '</a>
										<div class="featured_items_meta">
											<div class="preview_categ">'.__('in', 'theme_localization').' ' . $tempname . '</div>
