    ...
    foreach ($modules as $module_key => $module) {
        ...
        if ($module['name']=="bg_start" || $module['name']=="bg_end") {
            if ($module['name']=="bg_start") {
                $compile .= '<div class="module_line_trigger '.(isset($module['custom_class']) ? $module['custom_class'] : '').'" data-option="'.$module['properties'].'" data-background="'.((strlen($module['bg_color'])>0) ? "#".$module['bg_color'] : '').' '.((strlen($module['bg_image'])>0) ? "url('".$module['bg_image']."')" : '').' '.((strlen($module['properties']) == "stretch") ? 'no-repeat' : 'repeat').' 0 0" data-top-padding="'.$module['padding_top'].'" style="padding-bottom:'.$module['padding_bottom'].';">';
            }
            ...
            switch ($module['name']) {
                ...
                case "faq":
                    ...
                    $compile .= do_shortcode("[faq_shortcode
                    heading_alignment='".$module["heading_alignment"]."'
                    heading_color='".$module["heading_color"]."'
                    heading_size='".$module["heading_size"]."'
                    heading_text=\"".$module["heading_text"]."\"
                    ]".$faqgompile."[/faq_shortcode]");
                    unset($faqgompile);
                    break;
                #BREAK
